...
CREATE POLICY user_preferences_select_own ON user_preferences
  FOR SELECT USING (member_id = get_current_member_id());

CREATE POLICY user_preferences_insert_own ON user_preferences
  FOR INSERT WITH CHECK (member_id = get_current_member_id());

CREATE POLICY user_preferences_update_own ON user_preferences
  FOR UPDATE USING (member_id = get_current_member_id());

CREATE POLICY user_preferences_delete_own ON user_preferences
  FOR DELETE USING (member_id = get_current_member_id());

-- National admins can view all preferences
CREATE POLICY user_preferences_select_national_admin ON user_preferences
  FOR SELECT USING (is_national_admin_member());

-- =====================================================================================
-- RLS POLICIES: Dashboard Widgets
-- =====================================================================================

-- All authenticated members can view widgets they have permission for
CREATE POLICY dashboard_widgets_select_all ON dashboard_widgets
  FOR SELECT USING (is_authenticated_member());

-- Only national admins can manage widgets
CREATE POLICY dashboard_widgets_all_national_admin ON dashboard_widgets
  FOR ALL USING (is_national_admin_member());

-- =====================================================================================
-- RLS POLICIES: User Dashboard Layouts
-- =====================================================================================

-- Users can manage their own layouts
CREATE POLICY user_dashboard_layouts_select_own ON user_dashboard_layouts
  FOR SELECT USING (member_id = get_current_member_id());

CREATE POLICY user_dashboard_layouts_insert_own ON user_dashboard_layouts
  FOR INSERT WITH CHECK (member_id = get_current_member_id());

CREATE POLICY user_dashboard_layouts_update_own ON user_dashboard_layouts
  FOR UPDATE USING (member_id = get_current_member_id());

CREATE POLICY user_dashboard_layouts_delete_own ON user_dashboard_layouts
  FOR DELETE USING (member_id = get_current_member_id());

-- =====================================================================================
-- RLS POLICIES: User Dashboard Preferences
-- =====================================================================================

-- Users can manage their own widget preferences
CREATE POLICY user_dashboard_preferences_select_own ON user_dashboard_preferences
  FOR SELECT USING (member_id = get_current_member_id());

CREATE POLICY user_dashboard_preferences_insert_own ON user_dashboard_preferences
  FOR INSERT WITH CHECK (member_id = get_current_member_id());

CREATE POLICY user_dashboard_preferences_update_own ON user_dashboard_preferences
  FOR UPDATE USING (member_id = get_current_member_id());

CREATE POLICY user_dashboard_preferences_delete_own ON user_dashboard_preferences
  FOR DELETE USING (member_id = get_current_member_id());
...
-- Chapter admins can view refunds for their chapter members
CREATE POLICY refunds_select_chapter_admin ON refunds
  FOR SELECT USING (
    is_chapter_admin_member() AND
    EXISTS (
      SELECT 1 FROM members m
      WHERE m.id = refunds.member_id
      AND m.chapter_id = get_member_chapter_id()
    )
  );

-- State admins can view refunds in their state
CREATE POLICY refunds_select_state_admin ON refunds
  FOR SELECT USING (
    is_state_admin_member() AND
    EXISTS (
      SELECT 1 FROM members m
      WHERE m.id = refunds.member_id
      AND get_member_state(m.id) = get_member_state()
    )
  );

-- National admins have full access
CREATE POLICY refunds_all_national_admin ON refunds
  FOR ALL USING (is_national_admin_member());

-- =====================================================================================
-- RLS POLICIES: Event Expenses
-- =====================================================================================

-- Members can view expenses for events they created
CREATE POLICY event_expenses_select_creator ON event_expenses
  FOR SELECT USING (
    EXISTS (
      SELECT 1 FROM events e
      WHERE e.id = event_expenses.event_id
      AND e.created_by = get_current_member_id()
    )
  );

-- Chapter admins can manage expenses for their chapter events
CREATE POLICY event_expenses_all_chapter_admin ON event_expenses
  FOR ALL USING (
    is_chapter_admin_member() AND
    EXISTS (
      SELECT 1 FROM events e
      WHERE e.id = event_expenses.event_id
      AND e.chapter_id = get_member_chapter_id()
    )
  );

-- State admins can view expenses in their state
CREATE POLICY event_expenses_select_state_admin ON event_expenses
  FOR SELECT USING (
    is_state_admin_member() AND
    EXISTS (
      SELECT 1 FROM events e
      ...
      WHERE e.id = event_expenses.event_id
      AND c.state = get_member_state()
    )
  );

-- National admins have full access
CREATE POLICY event_expenses_all_national_admin ON event_expenses
  FOR ALL USING (is_national_admin_member());

-- =====================================================================================
-- RLS POLICIES: Chapter Metrics
-- =====================================================================================

-- Chapter admins can view their own chapter metrics
CREATE POLICY chapter_metrics_select_chapter_admin ON chapter_metrics
  FOR SELECT USING (
    (is_chapter_admin_member() AND chapter_id = get_member_chapter_id()) OR
    is_state_admin_member() OR
    is_national_admin_member()
  );

-- Only national admins can insert/update/delete metrics
CREATE POLICY chapter_metrics_modify_national_admin ON chapter_metrics
  FOR ALL USING (is_national_admin_member());

-- =====================================================================================
-- RLS POLICIES: Member Engagement
-- =====================================================================================

-- Members can view their own engagement
CREATE POLICY member_engagement_select_own ON member_engagement
  FOR SELECT USING (member_id = get_current_member_id());

-- System can insert engagement tracking for any member
CREATE POLICY member_engagement_insert_all ON member_engagement
  FOR INSERT WITH CHECK (true);

-- Admins can view engagement data
CREATE POLICY member_engagement_select_admin ON member_engagement
  FOR SELECT USING (
    is_chapter_admin_member() OR
    is_state_admin_member() OR
    is_national_admin_member()
  );

-- =====================================================================================
-- RLS POLICIES: Engagement Metrics
-- =====================================================================================

-- Members can view their own metrics
CREATE POLICY engagement_metrics_select_own ON engagement_metrics
  FOR SELECT USING (member_id = get_current_member_id());

-- Admins can view all metrics
CREATE POLICY engagement_metrics_select_admin ON engagement_metrics
  FOR SELECT USING (
    is_chapter_admin_member() OR
    is_state_admin_member() OR
    is_national_admin_member()
...
-- Only system/national admins can modify
CREATE POLICY engagement_metrics_modify_national_admin ON engagement_metrics
  FOR ALL USING (is_national_admin_member());

-- =====================================================================================
-- RLS POLICIES: Churn Predictions
-- =====================================================================================

-- Members cannot view their own churn predictions (prevent gaming)
-- Only admins can view predictions

CREATE POLICY churn_predictions_select_chapter_admin ON churn_predictions
  FOR SELECT USING (
    is_chapter_admin_member() AND
    EXISTS (
      SELECT 1 FROM members m
      WHERE m.id = churn_predictions.member_id
      AND m.chapter_id = get_member_chapter_id()
    )
  );

CREATE POLICY churn_predictions_select_state_admin ON churn_predictions
  FOR SELECT USING (
    is_state_admin_member() AND
    EXISTS (
      SELECT 1 FROM members m
...
CREATE POLICY churn_predictions_all_national_admin ON churn_predictions
  FOR ALL USING (is_national_admin_member());

-- =====================================================================================
-- RLS POLICIES: AI Analysis Log
-- =====================================================================================

-- National admins only
CREATE POLICY ai_analysis_log_all_national_admin ON ai_analysis_log
  FOR ALL USING (is_national_admin_member());
...
-- Users can view public cohorts or cohorts they created
CREATE POLICY cohorts_select_public ON cohorts
  FOR SELECT USING (
    is_public = true OR
    created_by = get_current_member_id() OR
    get_current_member_id() = ANY(shared_with) OR
    is_chapter_admin_member() OR
    is_state_admin_member() OR
    is_national_admin_member()
  );

-- Users can manage their own cohorts
CREATE POLICY cohorts_manage_own ON cohorts
  FOR ALL USING (created_by = get_current_member_id());
...
-- =====================================================================================
-- RLS POLICIES: Cohort Metrics
-- =====================================================================================

-- Follow cohort visibility rules
CREATE POLICY cohort_metrics_select_based_on_cohort ON cohort_metrics
  FOR SELECT USING (
    EXISTS (
      SELECT 1 FROM cohorts c
      WHERE c.id = cohort_metrics.cohort_id
      AND (
        c.is_public = true OR
        c.created_by = get_current_member_id() OR
        get_current_member_id() = ANY(c.shared_with) OR
        is_chapter_admin_member() OR
        is_state_admin_member() OR
        is_national_admin_member()
      )
...
-- Only national admins can modify metrics
CREATE POLICY cohort_metrics_modify_national_admin ON cohort_metrics
  FOR ALL USING (is_national_admin_member());
...
-- National admins have full access
CREATE POLICY notifications_all_national_admin ON notifications
  FOR ALL USING (is_national_admin_member());
...
-- Public documents are viewable by all authenticated members
CREATE POLICY documents_select_public ON documents
  FOR SELECT USING (
    access_level = 'public' AND
    is_authenticated_member() AND
    is_archived = false
  );

-- Members can view documents they uploaded
CREATE POLICY documents_select_own ON documents
  FOR SELECT USING (uploaded_by = get_current_member_id());

-- Chapter admins can manage their chapter documents
CREATE POLICY documents_all_chapter_admin ON documents
  FOR ALL USING (
    is_chapter_admin_member() AND
    (chapter_id = get_member_chapter_id() OR chapter_id IS NULL)
  );

-- State admins can view state-level documents
CREATE POLICY documents_select_state_admin ON documents
  FOR SELECT USING (
    is_state_admin_member() AND
    (
      ...
      EXISTS (
        ...
        WHERE c.id = documents.chapter_id
        AND c.state = get_member_state()
      )
    )
  );
...
-- National admins have full access
CREATE POLICY documents_all_national_admin ON documents
  FOR ALL USING (is_national_admin_member());
...
-- Public committees are viewable by all
CREATE POLICY committees_select_public ON committees
  FOR SELECT USING (
    is_public = true AND
    is_authenticated_member()
  );

-- Chapter admins can manage their chapter committees
CREATE POLICY committees_all_chapter_admin ON committees
  FOR ALL USING (
    is_chapter_admin_member() AND
    (chapter_id = get_member_chapter_id() OR chapter_id IS NULL)
  );

-- State admins can view state committees
CREATE POLICY committees_select_state_admin ON committees
  FOR SELECT USING (
    is_state_admin_member() AND
    (
      chapter_id IS NULL OR
      EXISTS (
        SELECT 1 FROM chapters c
        WHERE c.id = committees.chapter_id
        AND c.state = get_member_state()
      )
    )
  );
...
-- National admins have full access
CREATE POLICY committees_all_national_admin ON committees
  FOR ALL USING (is_national_admin_member());

-- =====================================================================================
-- INITIAL DATA: Dashboard Widgets
-- =====================================================================================

INSERT INTO dashboard_widgets (widget_key, name, description, category, component_name, min_width, min_height, icon, requires_role_tier)
VALUES
  ('member_growth_chart', 'Member Growth Chart', 'Trend chart showing member growth over time', 'analytics', 'MemberGrowthChart', 2, 2, 'TrendUp', 2),
  ('upcoming_events', 'Upcoming Events', 'List of upcoming events with registration status', 'events', 'UpcomingEventsList', 2, 2, 'CalendarDots', 1),
  ('revenue_summary', 'Revenue Summary', 'Financial overview with revenue breakdown', 'financial', 'RevenueSummary', 2, 2, 'CurrencyDollar', 3),
  ('engagement_score', 'Engagement Score', 'Member engagement metrics and trends', 'engagement', 'EngagementScore', 1, 1, 'ChartLine', 2),
  ('pending_renewals', 'Pending Renewals', 'Members with upcoming renewal deadlines', 'members', 'PendingRenewalsList', 2, 2, 'Clock', 2),
  ('recent_registrations', 'Recent Registrations', 'Latest event registrations', 'events', 'RecentRegistrations', 2, 2, 'UserPlus', 2),
  ('churn_risk_alerts', 'Churn Risk Alerts', 'Members at risk of churning', 'analytics', 'ChurnRiskAlerts', 2, 2, 'Warning', 3),
  ('quick_actions', 'Quick Actions', 'Frequently used actions and shortcuts', 'productivity', 'QuickActions', 1, 1, 'Lightning', 1),
  ('chapter_performance', 'Chapter Performance', 'Chapter metrics and benchmarking', 'analytics', 'ChapterPerformance', 3, 2, 'ChartBar', 3),
  ('notifications_widget', 'Notifications', 'Recent system notifications', 'system', 'NotificationsWidget', 1, 2, 'Bell', 1)
...
COMMENT ON SCHEMA public IS 'NABIP AMS - Issues #61-#91: User preferences, financial tracking, analytics, and infrastructure tables added on 2025-01-15';
